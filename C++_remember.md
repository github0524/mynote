...
![image-20230602143339603](image/C++牢记.assets/image-20230602143339603.png)

运算符的优先级[ ]是大于*的，所以如果是指向int 型的数组是需要加上括号的



### 多维数组作为形参

下面这个多维数组，是指向第一行第一个元素的指针，这整个多维数组形参会被编译器识别出一个指向int型的10个size长的指针，这里还有一个概念是C++中不存在多维数组，只是数组的成员是数组，而matrix[ ]表示的是数组的第一个元素，这里第一个元素里面就是一个10*int的数组

![image-20230602145024346](image/C++牢记.assets/image-20230602145024346.png)

main函数的参数

char * argv[ ]这个参数应该理解成一个数组，size未知，而数组内存储的类型是char*，就是说每个元素都是一个字符串

这个参数也可以写成char **argv,[ ]优先级比 * 大

![image-20230602151024989](image/C++牢记.assets/image-20230602151024989.png)
...
也就是说，数组作为参数的时候，并不能进行值传递，传递的仅仅是指向数组首元素的指针

**形参的写法**，以下三种形参写法都是等价于const int *，只是写成了数组形式，实际会转化

![image-20230602135700991](image/C++牢记.assets/image-20230602135700991.png)

**实参的写法**，实际传递的也是指针类型

![image-20230602135816523](image/C++牢记.assets/image-20230602135816523.png)



上面这些限制，说明传入了首指针，但是会有数组越界的问题

有三种检测方法：

1.C风格字符串，判断是否空指针，判断*P的内容是不是空字符串，因为C风格字符串有明显的结束标志

2.使用标准库规范，就是函数的形参设置成，首指针，和尾后指针

3.传递的参数中加上一个表示数组大小的形参
